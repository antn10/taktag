    ...
    <div id="collapseOne" class="accordion-collapse collapse" aria-labelledby="headingOne" 
        ...
        <div class="accordion-body">
            <div class="input-group mb-3 disabled"><span class="input-group-text">Nombre</span>
                <input type="text" class="form-control" disabled name="editNombre" id="editNombre" required>
            </div>
            <div class="input-group mb-3 disabled"><span class="input-group-text">Descripción</span>
                <textarea class="form-control" id="editDesc" rows="3" disabled></textarea>
            </div>
            <div><input class="form-control" type="file" id="formFile" disabled></div>
            <br>
            <div class="input-group mb-3 disabled">
                <span class="input-group-text">Serie</span>
                <input type="text" class="form-control" id="editSerie" required disabled>
                <button class="btn btn-primary" type="button" id="button-addon2">...</button>

                <span class="input-group-text">Marca</span>
                <input type="text" class="form-control" id="editMarca" required disabled>
                <button class="btn btn-primary" type="button" id="button-addon2">...</button>
            </div>
            <div class="input-group mb-3">
                <span class="input-group-text">Modelo</span>
                <input type="text" class="form-control" id="editModelo" required disabled>
                <button class="btn btn-primary" type="button" id="button-addon2">...</button>

                <span class="input-group-text">Clase</span>
                <select class="form-select" id="editClase" disabled>
                    <?php QryOptions('SELECT id,titulo nombre FROM clasificacion ORDER BY id',$alm);?>
                </select>
            </div>        
            <div class="input-group mb-3" >
                <button class="btn btn-primary" type="button" data-bs-toggle="modal" 
                data-bs-target="#modalAdd">editar</button>&nbsp;
            </div>
        </div>
    </div>
</div>
...
<?php $sql = "select serie, descripcion, almacen from elementos ele where art=$prod ";
    QryTabla($sql,['serie','descripcion'],'','');
